PIPO Core version 3.0.5 is now available from:
...
PIPO Core v3.0.5 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zPIPO protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI. Users are required to update before block `908000` which is when the accumulators will be refactored. Shortly after that block, zPIPO transactions will be enabled. **When zPIPO is enabled, autominting will also be enabled.** If you would like to disable automatic conversion of 10% of your PIPO balance to zPIPO, then you will need to add `enablezeromint=0` to your `pipo.conf` file. For information about where to find your pipo.conf you can visit this link from [PIPO Support](https://pipo.freshdesk.com/support/solutions/articles/30000004664-where-are-my-wallet-dat-blockchain-and-configuration-conf-files-located-).
...
---------------------
Handling the above noted libzerocoin exploit required the PIPO team to immediately release a patched wallet to as many users as possible which rejected bad spends and also disabled all zPIPO transactions in general. The process of releasing a patched wallet in such a small time frame is frustrating and difficult for all members of the PIPO team and especially users of PIPO. The PIVX developers have added a new spork which allows for zPIPO transacting to be turned on/off without having to release a patched wallet. This will allow much smoother operation if any problems occur in the future, and should also allow exchanges and 3rd party services to continue to operate even if zPIPO is in maintenance mode.
