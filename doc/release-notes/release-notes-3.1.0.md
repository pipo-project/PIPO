PIPO Core version *3.1.0* is now available from:  <https://github.com/pipo-project/PIPO/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/pipo-project/PIPO/issues>


Mandatory Update
==============

PIPO Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zPIPO Staking is here! zPIPO staking will be activated on the morning of the 8th of May 2018. With the release of zPIPO staking, there are effectively 2 versions of zPIPO, zPIPO minted on the 3.0.6 PIPO wallet or lower, and zPIPO minted on PIPO wallet version or higher. New features in this release will require the use of zPIPO v2, zPIPO minted on this wallet release 3.1.0 or later. If you currently hold zPIPO v1 and wish to take advantage of zPIPO staking and deterministic zPIPO, you will need to spend the zPIPO v1 to yourself and remint zPIPO v2.
...
To encourage the use of zPIPO and increase the PIPO zerocoin anonymity set, the PIPO payment schedule has been changed to the following:
...
The visual layout of the PIPO Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zPIPO minting feature. This is enabled by default and the enablezeromint=0 setting in the pipo.conf file will overwrite the GUI option.
...
### Summary
 ...
 The PIPO wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the pipo.conf file, found in the PIPO data directory.
...
PIPO Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The PIPO client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running pipod without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the PIPO support group, which often times sees users say "I didn't spend that zPIPO". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the PIPO masternodes, the monthly budget available to be utilised has been increased to 42,000 PIPO / month. This PIPO only has the opportunity to be raised once per month (paid to winning proposals) with any unused PIPO not created by the blockchain.

### Budget Finalization Fee

The PIPO finalization fee for successful proposals has now been reduced, this fee is now 5 PIPO down from 50 PIPO. The total fee outlay for a successful proposal is now a total of 55 PIPO.
...
-	Fixed a  bug that would not start masternodes from the PIPO-Qt masternodes tab
-	Updated PIPO-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and PIPO's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zPIPO Control dialog in the QT wallet, a user is able to see zPIPO marked as version 1 or 2.
...
PIPO's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which PIPO used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to PIPO using the bug bounty program. The modulus is now correctly imported and PIPO's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault pipo-cli getinfo while loading block index (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/pipo-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
