...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePipoAmount"))
        settings.setValue("nAnonymizePipoAmount", 1000);

    nAnonymizePipoAmount = settings.value("nAnonymizePipoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePipoAmount"))
        SoftSetArg("-anonymizepipoamount", settings.value("nAnonymizePipoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pipo.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePipoAmount:
            return QVariant(nAnonymizePipoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizePipoAmount:
            nAnonymizePipoAmount = value.toInt();
            settings.setValue("nAnonymizePipoAmount", nAnonymizePipoAmount);
            emit anonymizePipoAmountChanged(nAnonymizePipoAmount);
            break;
        case CoinControlFeatures:
